...
// Copyright (c) 2011-2013 The Negativity developers
...
const string strMessageMagic = "Negativity Signed Message:\n";
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // Negativity: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // Negativity: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Negativity: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Negativity: find block wanted by given orphan block
...
// Negativity: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// Negativity: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (pindexPrevPrev->pprev == NULL)
		...
		if (!fProofOfStake && pindexLast->nHeight > 0 && pindexLast->nHeight < 1000)
        return bnProofOfWorkLimit.GetCompact();

    int64 nActualSpacing = pindexPrev->GetBlockTime() - pindexPrevPrev->GetBlockTime();

    // Negativity: target change every block
    // Negativity: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // Negativity: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Negativity: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // Negativity: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // Negativity: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Negativity: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Negativity: track money supply and mint amount info
    ...
    // Negativity: fees are not collected by miners as in bitcoin
    // Negativity: fees are destroyed to compensate the entire network
...
// Negativity: total coin age spent in transaction, in the unit of coin-days.
...
// Negativity: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // Negativity: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // Negativity: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // Negativity: record proof-of-stake hash value
    ...
    // Negativity: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // Negativity: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // Negativity: coinbase output should be empty if proof-of-stake block
    ...
    // Negativity: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // Negativity: check that the block satisfies synchronized checkpoint
    ...
    // Negativity: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Negativity: check proof-of-stake
    ...
    // Negativity: verify hash target and signature of coinstake tx
    ...
    // Negativity: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // Negativity: getblocks may not obtain the ancestor block rejected
    ...
    // Negativity: if responsible for sync-checkpoint send it
...
// Negativity: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "Negativity", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Negativity", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Negativity: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // Negativity: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // Negativity: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // Negativity: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // Negativity: should not enter safe mode for longer invalid chain
    // Negativity: if sync-checkpoint is too old do not enter safe mode
    if (Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 10) && !fTestNet)
    {
        nPriority = 100;
        strStatusBar = strRPC = "The blockchain is currently downloading and verifying blocks; do not create any new transactions until you are fully synced";
    }

    // Negativity: if detected invalid checkpoint enter safe mode
    ...
    // Negativity: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // Negativity: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // Negativity: record my external IP reported by peer
        ...
        // Negativity: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // Negativity: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Negativity: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // Negativity: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Negativity: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // Negativity: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
